...
def _files_to_ds(f):
    ...
    ds = ds.map(_preprocess_once, num_parallel_calls=tf.data.experimental.AUTOTUNE).window(40, 30)
    ds = ds.flat_map(lambda x,y: tf.data.Dataset.zip((x, y)).batch(40)).batch(1)
    return ds
