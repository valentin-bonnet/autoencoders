...
class KVAE(tf.keras.Model):
    def __init__(self, layers=[64, 128, 512], latent_dim=1024, input_shape=64, sequence_length=20, dim_a=100, dim_z=2, dim_u=10, std=0.05, use_bn=False):
    ...
    def get_loss(self, im):
        ...
        KL = tfp.distributions.kl_divergence(mvn_a, tfp.distributions.MultivariateNormalTriL(0, tf.linalg.cholesky(tf.eye(self.dim_z, batch_shape=[self.batch_size*self.seq_size]))))
