...
class Memory(tf.keras.layers.Layer):
    ...
    def call(self, inputs, states):
        ...
        attention_k = tf.gather_nd(tf.reshape(k, [self.k_shape, self.batch_shape, self.hw_shape]), top_indx, batch_dims=0)  # (bs, top_a, k)
        print("attention_k.shape: ", attention_k.shape)
        attention_v = tf.gather_nd(tf.reshape(v, [self.v_shape, self.batch_shape, self.hw_shape]), top_indx, batch_dims=0)  # (bs, top_a, v)
