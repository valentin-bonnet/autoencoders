...
class Memory(tf.keras.layers.Layer):
    ...
    def call(self, inputs, states):
        ...
        attention_k = tf.gather_nd(tf.reshape(k, [self.batch_shape, self.k_shape, self.hw_shape]), top_indx, batch_dims=0)  # (bs, top_a, k)
        print("attention_k.shape: ", attention_k.shape)
        attention_v = tf.gather_nd(tf.reshape(v, [self.batch_shape, self.v_shape, self.hw_shape, ]), top_indx, batch_dims=0)  # (bs, top_a, v)
        attention_k = tf.reshape(attention_k, [self.batch_shape, self.hw_shape, self.k_shape])
        attention_v = tf.reshape(attention_v, [self.batch_shape, self.hw_shape, self.v_shape])
