...
def KAST_test(kast, davis, file_name_head='image', path='./'):
    ...
    seq_size = output_v.shape[0]
    all_white = np.ones([1, output_v.shape[1], output_v.shape[2], 3], dtype=np.uint8) * [255, 255, 255]
    ...
    file_path = os.path.join(path, file_name_head)
    im[0].save(file_path + '_DAVIS.gif', save_all=True, append_images=im[1:], duration=50)
