...
def KAST_View(kast, input_data, file_name_head='image', path='./'):
    ...
    for i in range(seq_size):
        ...
        ground_truth[i] = cv2.cvtColor(np.int32((ground_truth[i].numpy() + 1.0) * [50.0, 127.5, 127.5] - [0., 128., 128.]), cv2.COLOR_Lab2RGB)
        output[i] = cv2.cvtColor(np.int32((output[i].numpy() + 1.0) * [50.0, 127.5, 127.5] - [0., 128., 128.]), cv2.COLOR_Lab2RGB)
        image_drop_out[i] = cv2.cvtColor(np.int32((image_drop_out[i].numpy() + 1.0) * [50.0, 127.5, 127.5] - [0., 128., 128.]), cv2.COLOR_Lab2RGB)
